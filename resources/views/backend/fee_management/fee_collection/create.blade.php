    ...
    @section('title','Add New Fee Collection')
    @section('content')
                    ...
                    <div class="col-xl-12">
                        <div class="panel panel-inverse" data-sortable-id="form-stuff-10">
                            <div class="panel-heading">
                                <h4 class="panel-title">Add New Fee Collection</h4>
                            ...
                            <div class="panel-body">
                                <form action="{{ route('admin.fee-collection.store') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <div class="row">
                                        ...
                                        <button type="submit" class="submitButton btn btn-sm btn-primary pull-right">Submit</button>
    ...
    /*get batch id by student id , class id and session id*/
        ...
        $(document).on('change keyup click','.dropdown_class,.studentName,.student_id,.class_id,.session_id,.batch_setting_id,.fee_cat_id,.batch_type_id,.month_id,.year',function(){
            $('.submitButton').attr('disabled','disabled');
            getStudentCurrentData();
        });
        ...
        function getStudentCurrentData()
        {
            ...
            if(student_id && class_id && session_id && fee_cat_id && month_id && batch_setting_id && batch_type_id)
            {
                $.ajax({
                    type: "get",
                    url: "{{ route('admin.fee_collection_getFeeCategoryAmount') }}",
                    data: {student_id:student_id ,class_id:class_id,
                            session_id:session_id,fee_cat_id:fee_cat_id,
                            month_id:month_id,batch_setting_id:batch_setting_id ,batch_type_id:batch_type_id
                        },
